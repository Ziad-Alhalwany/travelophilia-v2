...
- **تخصيص صلاحيات الرموز (JWT Custom Claims):** يتم حقن الصلاحيات الهيكلية للموردين داخل حزم التحقق لتوكن JWT (Access/Refresh Tokens) عبر الحقول `"scopes"` (مثل `["properties:read", "properties:write", "inventory:sync"]`) و `"vendor_id"` للتحقق والمنع التلقائي على مستوى الخادم.
- **تتبع مؤشرات الأجهزة والعملاء (Client Telemetry):** للحد من مشاركة حسابات الشركاء والعمليات الاحتيالية B2B، يتم جمع بيانات العميل غير الشخصية (non-PII) مثل أبعاد الشاشة (screen dimensions)، والمنطقة الزمنية للعميل (timezone)، وبيانات المتصفح ونظام التشغيل (browser metadata) وربطها بسجل الدخول.
- **مرونة الاتصال غير المتصل (PWA Connection Interceptors):** يراقب النظام حالة الشبكة عبر `navigator.onLine` مع تفعيل مقاطعات شبكة خاصة (Resilience Interceptors) تضمن حفظ أمن الجلسات والانتقال لوضع عدم الاتصال (Offline Session Fallback) وتهيئة تجربة التشغيل بشكل آمن وتدريجي دون انهيار أو تسريب بيانات الجلسة.
