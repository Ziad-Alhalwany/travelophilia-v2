...
class TripRequestGenerateCodeView(APIView):
    """
    Public endpoint to generate a unique trip code.
...
class TripRequestCRMListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        allowed = {"created_at", "-created_at", "next_followup_at", "-next_followup_at", "priority", "-priority"}
...
class TripRequestNoteListCreateView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        return TripRequestNote.objects.filter(trip_request_id=self.kwargs["pk"]).select_related("created_by")
